...
public:
    ...
    void findturnaround(int pid[], int bt[], int n, int tat[], int pri[])
    {
        int t = 0, copy[n], c = 0; // copy -> copy of priority times
        bool flag = true;
        for (int i = 0; i < n; i++)
        {
            copy[i] = pri[i];
    ...
    void findaveragetime(int pid[], int bt[], int n, int pri[])
    {
        double s = 0.0, avgwt, avgtat;
        int wt[n], tat[n];
        findturnaround(pid, bt, n, tat, pri);
...
int main()
{
    ...
    obj.findaveragetime(pid, bt, n, pri);
}